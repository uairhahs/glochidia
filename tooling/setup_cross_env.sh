...
find "$SOURCE_DIR" -type f \( -name "getopt.c" -o -name "fnmatch.c" -o -name "getopt1.c" \) -print0 | while IFS= read -r -d $'\0' file; do
...
find "$SOURCE_DIR" -type f -name "getopt.h" -print0 | while IFS= read -r -d $'\0' file; do
...
find "$SOURCE_DIR" -type f -name "getopt.c" -print0 | while IFS= read -r -d $'\0' file; do
...
# --- Patch 4 (Robust Fix): Fix incorrect C-style function pointer casting in io.c (gawk/musl issue) ---
echo "  -> Applying robust patch for incompatible function pointer casting in io.c."
...
find "$SOURCE_DIR" -type f -name "io.c" -print0 | while IFS= read -r -d $'\0' file; do
